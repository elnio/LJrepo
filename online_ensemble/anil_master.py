...
def run_simple(rf, last_x_chunk, last_y_chunk, reader):
    ...
    SS = 0.001 # Scaled step-size
    lambda_E0 = 0.003 # Forgetting factor for the cross correlation
    ...
    for i in range(chunk_size, chunk_size + n_test_data):
        ...
        # print the result
        print 'i = {0}\tpre = {1:.5f}\ttar = {2:.5f}\tmse = {3:.5f}\tss = {4:.3f}\tbias = {5:.3f}\tM ={6}\tsw ={7}'.format(i, predict, y, mse / (i + 1 - chunk_size), ss, w0['intercept'], N_trees, sum(w0.values()))        
        mse += (predict - y) ** 2
        ...
        for w in range(ws-1):
            ff1[w] = ff1[w+1]
        ff1[ws-1] = norm(results[idx] * ss * (y - predict))        
        f11 += 1
        f1 = (mean(ff1[0:ws-1]) < norm(w0.values())*wsr) 
        ...
        if f2:
            if (add_trees > -ceil(N_trees/20)) and ( N_trees + add_trees > n_trees/4):
                add_trees += -ceil(N_trees/100)
#        else:
#            add_trees += floor(N_trees/50)
            
        # replace trees 
        if ((f1) or (add_trees < -N_trees/10) )and (f11>0):
            f11 = - chunk_size/2 #floor(N_trees/5)
            if (f1) and (~f2) and ( N_trees + add_trees < n_trees*2):
                add_trees += ceil(N_trees/10)
#            if (f1) and (f2):
#                add_trees += -floor(N_trees/10)
            ...
            threshold = min([(np.percentile(cros,50))*0.5 , (np.percentile(cros,30))] )         
            for idx in w0:
